...
public class Message
{
	...
	public Message(char inMesType, String inStringData)
	{
	...
	public Message(char inMesType, int inBoxID, int inWWID)
	{
		senderType = APP_SENDER_CHAR;
		mesType = inMesType;
		boxID = inBoxID;
	...
	public Message(char inMesType, int inBoxID, int inDrawerID, int inSensorID, int inWWID)
	{
		senderType = APP_SENDER_CHAR;
		mesType = inMesType;
		boxID = inBoxID;
	...
	//app register 
	public Message(char inMesType, int inWWID, String inStringData)
	{
		senderType = APP_SENDER_CHAR;
		aWWID = inWWID;
		stringData = inStringData;
	...
	public Message (String inData)
	{
		...
		if (headerTest.equals(HEADER_STRING))
		{
			...
			if (stringLength > 0)
				stringData = inData.substring(STRING_POSITION,STRING_POSITION+stringLength);
		}
		else
		{
	...
	public String toString()
	{
		...
		if (stringData.length() > STRING_SIZE)
			newMessage = newMessage + stringData.substring(0,STRING_SIZE);
		else
